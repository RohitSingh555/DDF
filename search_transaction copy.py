...
def update_database_row(table_element, cursor):
    table_name = 'property_listings_test_table1'
...
def delete_records_not_matching(xml_ids):
    ...
    query = "SELECT ID FROM property_listings_test_table1"
    ...
    # if ids_to_delete:
    #     delete_query = "DELETE FROM property_listings_test_table1 WHERE ID IN (%s)"
...
def check_different_last_updated(intersection_ids, xml_property_updates):
    ...
    for id in intersection_ids:
        query = f"SELECT LastUpdated FROM property_listings_test_table1 WHERE ID = '{id}'"
        cursor.execute(query)
